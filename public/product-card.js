class ProductCard extends HTMLElement {
  ...
  render() {
    ...
    this.innerHTML = `
      <article
        class="product-card"
        data-signals__ifmissing="{name${i}: '', description${i}: '', price${i}: 0, cartPrice${i}: 0, cartOriginalPrice${i}: 0, qty${i}: 1, inCart${i}: false, originalPrice${i}: 0, visible${i}: false, countdown${i}: 300, saleRestart${i}: 0}"
        ...
        data-on-interval="($countdown${i} > 0 && $countdown${i}--), ($countdown${i} === 0 && $originalPrice${i} > 0 && $saleRestart${i} === 0 && ($price${i} = $originalPrice${i}, $originalPrice${i} = 0, $saleRestart${i} = 20)), ($saleRestart${i} === 1 && @get('/products/refresh')), ($saleRestart${i} > 0 && $saleRestart${i}--)"
      >
        ...
        <div class="card-details" data-class="{collapsed: $inCart${i}}">
          ...
          <button
            class="add-to-cart"
            data-on:click="$cartPrice${i} = $price${i}, $cartOriginalPrice${i} = $originalPrice${i}, $cartCount++, $inCart${i} = true, @post('/cart/add/${i}')"
